...
def import_model_file(request):
    try:
        ...
        return Response('Whoops, this isnt right. Check the spec and try again.', status=status.HTTP_400_BAD_REQUEST)
# EV 4 updates:
...
def import_asset_file(request):
    try:
        ...
        return Response('An error occurred during the import. Please check the bulk spec and try again.', status=status.HTTP_400_BAD_REQUEST)
...
def import_network_port_file(request):
    try:
        ...
        return Response('Sorry, there was an eror during that import. Please double check and ensure the file matches the import spec.', status=status.HTTP_400_BAD_REQUEST)
